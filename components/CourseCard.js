...
export default function CourseCard({ course, registerPath = '/' }) {
  ...
  return (
    <div className={`rounded-2xl border p-5 flex flex-col gap-4 bg-white transition-shadow hover:shadow-md ${
      ...
      <ul className="space-y-1.5 text-sm text-gray-700">
        ...
        <li className="flex items-start gap-2">
          <Icon name="location_on" size={14} className="text-gray-400 flex-shrink-0 mt-0.5" />
      ...
      {isOpen ? (
        <Link
          href={ctaHref}
          className="w-full text-center py-3 rounded-xl bg-brand-700 text-white font-semibold text-sm hover:bg-brand-800 transition-colors"
        >
          Đăng ký ngay
        </Link>
      ) : (
        <span className="w-full text-center py-3 rounded-xl bg-gray-100 text-gray-400 font-semibold text-sm cursor-not-allowed block">
          Đã đủ học viên
        </span>
      )}
    </div>
